...
getTabTitle() { const map: any = { dashboard: '主控台 Dashboard', orders: '訂單管理 Orders', products: '商品管理 Products', customers: '客戶管理 Customers', accounting: '銷售報表 Accounting', inventory: '庫存盤點 Inventory', purchases: '採購總帳 Purchases', settings: '商店設定 Settings' }; return map[this.activeTab()] || ''; } 
  ...
  // ☁️ 同步至 Google Sheets：採購總帳
  async syncPurchasesToGoogleSheets() {
    // 這裡我們直接共用你剛剛拿來接「銷售報表」的那個 GAS 網址！
    // GAS 腳本非常聰明，只要我們傳的 sheetName 不一樣，它就會自動幫你在同一個 Excel 檔案裡開一個「新分頁」！
    ...
    try {
      const btnText = confirm(`⏳ 準備將 ${payloadRows.length} 筆採購單據同步至 Google Sheets\n\n按下「確定」開始傳送。`);
      if (!btnText) return;
      
      const response = await fetch(SHEETS_GAS_URL, {
        method: 'POST',
        body: JSON.stringify({ 
          sheetName: '採購總帳', // 👈 這裡告訴 GAS 要把資料塞進「採購總帳」分頁
          rows: payloadRows 
        })
      });
      ...
      if (result.success) {
        alert('✅ ' + result.message + '\n您的 Google Sheets 已自動新增「採購總帳」分頁了！');
      } else {
        alert('❌ 同步失敗: ' + result.error);
      }
    } catch (error) {
      console.error(error);
      alert('❌ 發生網路錯誤，無法連線至 Google Sheets API。');
    }
  }  
...
exportInventoryCSV() { 
  ...
  private async pushToGoogleSheets(sheetName: string, rows: any[]) {
    ...
    if (!confirm(`⏳ 準備將 ${rows.length} 筆資料同步至 Google Sheets 的「${sheetName}」分頁\n\n按下「確定」開始傳送。`)) return;
    try {
  ...
  syncOrdersToGoogleSheets() {
    ...
    this.pushToGoogleSheets('訂單管理', rows);
  }
  ...
  syncProductsToGoogleSheets() {
    ...
    this.pushToGoogleSheets('商品總表', rows);
  }
  ...
  syncCustomersToGoogleSheets() {
    ...
    this.pushToGoogleSheets('會員名單', rows);
  }
  ...
  syncInventoryToGoogleSheets() {
    ...
    this.pushToGoogleSheets('庫存盤點表', rows);
  }
  ...
  syncProcurementToGoogleSheets() {
    ...
    this.pushToGoogleSheets(`叫貨表_${rangeLabel}`, rows);
  }
  exportToCSV() { 
    const range = this.accountingRange(); 
    const now = new Date(); 
